// components/prieres/PriereFormDialog.tsx
'use client';
...
export default function PriereFormDialog({
  open,
  priere,
  onClose,
  onSuccess,
  ...
  const [priority, setPriority] = useState<PrierePriority>('NORMAL');
  ...
  useEffect(() => {
    if (!open) return;
    if (priere) {
      setTitle(priere.title);
      setDate(toLocalInput(priere.date));
      setLocation(priere.location ?? '');
      setDetail(priere.detail ?? '');
      setPriority(priere.priority);
      setNotification(priere.notification); // 🔔 AJOUT
    } else {
      setTitle('');
      setDate('');
      setLocation('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing) {
        await prieresService.update(priere!.id, payload);
      } else {
        await prieresService.create(payload);
      }
      onSuccess();
    } catch (err: any) {
      const msg =
      ...
      >
        ...
        >
          <div>
            ...
            >
              {isEditing ? 'Modifier la prière' : 'Nouvelle prière'}
            </h2>
            <p
              className="mt-0.5 text-xs"
              style={{ color: colors.textSecondary }}
            >
              Le titre est obligatoire. Le reste est optionnel.
            </p>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-5">
          <Field label="TITRE *">
            <input
              ...
              placeholder="Prière pour la nation"
            />
          </Field>
          ...
          <Field label="DÉTAIL (optionnel)">
            <textarea
              ...
              placeholder="Unissons-nous pour prier…"
            />
          </Field>

          <Field label="PRIORITÉ">
            <div className="flex gap-2">
              {(['NORMAL', 'IMPORTANT', 'URGENT'] as PrierePriority[]).map(
                (p) => {
